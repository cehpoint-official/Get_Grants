                ...
                <>
                    <div className="bg-white border rounded-lg shadow-sm mb-6 p-4">
                        <div className="flex justify-between items-center">
                            ...
                            <Button onClick={() => { setEditingGrant(null); setShowGrantModal(true); }} className="bg-violet text-white hover:bg-pink font-medium">+ Create Grant</Button>
                        </div>
                    </div>
            ...
            case "Applications": return (
                <div>
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <Button onClick={() => exportToExcel(applications, "GrantApplications")} className="bg-green-600 hover:bg-green-700 text-white" disabled={applications.length === 0}>Export to Excel</Button>
                    </div>
                        ...
                        <table className="w-full text-sm text-left text-gray-500">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" className="px-6 py-3">Help Needed</th>
                                    <th scope="col" className="px-6 py-3">Support Areas</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                {premiumInquiries.map((inquiry) => (
                                    <tr key={inquiry.id} className="bg-white border-b hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center text-xs"><Mail className="w-3 h-3 mr-2 text-gray-400" />{inquiry.email}</div>
                                            <div className="flex items-center text-xs mt-1"><Phone className="w-3 h-3 mr-2 text-gray-400" />{inquiry.phone || 'N/A'}</div>
                                        </td>
                                        <td className="px-6 py-4">{inquiry.companyName || "N/A"}</td>
                                        <td className="px-6 py-4 max-w-xs"><div className="line-clamp-3 text-gray-700 text-xs whitespace-pre-wrap">{inquiry.specificNeeds || inquiry.message || ""}</div></td>
                                        <td className="px-6 py-4"><span className={`capitalize px-2 py-1 text-xs font-semibold rounded-full`}>{inquiry.status}</span></td>
                                        <td className="px-6 py-4 flex gap-2">
                                            <Button size="sm" variant="outline" onClick={() => setReplyInquiry(inquiry)}>Reply</Button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                        {premiumInquiries.length === 0 && (<div className="text-center py-8 text-gray-500">No user queries yet.</div>)}
                    </div>
                </div>
            );
            case "Contact Messages": return (
                <div>
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <Button
                            onClick={() => exportToExcel(contactMessages as any, "ContactMessages")}
                        ...
                        <table className="w-full text-sm text-left text-gray-500">
                            ...
                            <tbody>
                                {contactMessages.map((msg) => (
                                    <tr key={msg.id} className="bg-white border-b">
                                        <td className="px-6 py-4">{msg.createdAt.toLocaleDateString()}</td>
                ...
                <>
                    <div className="bg-white border rounded-lg shadow-sm mb-6 p-4">
                        <div className="flex justify-between items-center">
                            ...
                            <Button onClick={() => { setEditingPost(null); setShowModal(true); }} className="bg-violet text-white hover:bg-pink font-medium">+ Create Blog</Button>
                        </div>
                    </div>
                    <div className="bg-white border rounded-lg shadow-sm mb-6 p-4">
                        <h3 className="text-lg font-semibold mb-3 text-[#EB5E77]">Pending Approval ({pendingPosts.length})</h3>
                                    ...
                                    <div key={p.id} className="border border-[#EB5E77]/20 rounded-lg p-4 bg-[#FFE1E0]/10">
                                        <div className="flex items-start justify-between">
                                            <div className="pr-4 flex-1">
                                                ...
                                                <div className="text-sm text-[#565F6C] mb-2">By <span className="font-medium">{p.authorName || p.author}</span>{p.authorEmail && <span> • {p.authorEmail}</span>}</div>
                                            ...
                                            <div className="flex flex-col gap-2 ml-4">
                                                <Button size="sm" variant="outline" onClick={() => { setActivePendingPost(p); setShowPendingModal(true); }} className="text-[#EB5E77] border-[#EB5E77] hover:bg-[#EB5E77] hover:text-white">View</Button>
                        ...
                        ) : (<div className="text-center py-8 text-[#565F6C]"><p>No pending blog posts for approval.</p></div>)}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {posts.map((post) => (
                ...
                <>
                    <div className="bg-white border rounded-lg shadow-sm mb-6 p-4">
                        <div className="flex justify-between items-center">
                            ...
                            <Button onClick={() => { setEditingEvent(null); setShowEventModal(true); }} className="bg-violet text-white hover:bg-pink font-medium">+ Add Event</Button>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="bg-white border rounded-lg shadow-sm p-4 lg:col-span-1">
                            <Calendar mode="single" selected={selectedDate} onSelect={(d: any) => setSelectedDate(d || new Date())} className="rounded-md" modifiers={{ hasEvent: events.map((ev) => new Date(ev.start)) }} modifiersClassNames={{ hasEvent: "bg-violet/20 text-violet rounded-full", }} />
                        </div>
                        <div className="bg-white border rounded-lg shadow-sm p-4 lg:col-span-2">
                            ...
                            <div className="space-y-3">
                                {eventsForSelectedDate().length === 0 && (<p className="text-sm text-gray-500">No events for this day.</p>)}
                    ...
                    <div className="bg-white border rounded-lg shadow-sm p-4 mt-6">
                        ...
                        {events.length === 0 ? (<p className="text-sm text-gray-500">No events added yet.</p>) : (
