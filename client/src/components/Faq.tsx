...
import { Plus, X, User, Mail, Phone, Building, MessageSquare } from "lucide-react";
        ...
        <div id="faq" className="py-16 sm:py-24" style={{ background: ' #FFFFFF ' }}>
            <div className="container mx-auto px-4">
                            ...
                            {faqs.map((faq, index) => (
                                <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden">
                                    <button onClick={() => handleFaqToggle(index)} className="w-full flex justify-between items-center text-left p-5">
                                        <span className="text-base font-medium" style={{ color: '#30343B' }}>{faq.question}</span>
                                    ...
                                    <div className={`overflow-hidden transition-all duration-300 ease-in-out ${openFaqIndex === index ? "max-h-screen" : "max-h-0"}`}>
                                        <div className="px-5 pb-5">
                                            <p style={{ color: '#565F6C' }}>{faq.answer}</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
                ...
                <div className="text-center mt-16">
                    <Button onClick={() => setIsContactModalOpen(true)} className="bg-[linear-gradient(90deg,_#8A51CE_0%,_#EB5E77_100%)] hover:opacity-90 text-white px-4 py-4 font-semibold text-base rounded-lg shadow-lg transition-opacity" style={{ backgroundColor: '#EB5E77' }}>
                        Have Any Queries
                    </Button>
                </div>
            </div>
                    ...
                    <div className="bg-white p-8 rounded-2xl shadow-2xl relative w-full max-w-md max-h-[90vh] overflow-y-auto">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                            <div className="relative">
                                ...
                                <input type="text" placeholder="Your Name *" {...register("name", { required: "Name is required" })} className="w-full p-3 pl-10 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-0 focus:border-[#EB5E77] transition-colors" />
                                {errors.name && <p className="text-red-500 text-sm mt-1">{errors.name.message}</p>}
                            </div>
                            <div className="relative">
                                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
                                <input type="email" placeholder="Your Email *" {...register("email", { required: "Email is required", pattern: { value: /^\S+@\S+$/i, message: "Invalid email" } })} className="w-full p-3 pl-10 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-0 focus:border-[#EB5E77] transition-colors" />
                                {errors.email && <p className="text-red-500 text-sm mt-1">{errors.email.message}</p>}
                            </div>
                            <div className="relative">
                                <Phone className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
                                <input type="tel" placeholder="Phone Number *" {...register("phone", { required: "Phone number is required" })} className="w-full p-3 pl-10 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-0 focus:border-[#EB5E77] transition-colors" />
                                {errors.phone && <p className="text-red-500 text-sm mt-1">{errors.phone.message}</p>}
                            </div>
                            <div className="relative">
                                <Building className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
                                <input type="text" placeholder="Company/Startup Name *" {...register("companyName", { required: "Company Name is required" })} className="w-full p-3 pl-10 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-0 focus:border-[#EB5E77] transition-colors" />
